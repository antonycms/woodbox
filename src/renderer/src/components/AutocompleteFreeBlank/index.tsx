...
import styles from '../AutocompleteBlank/styles.module.css';
...
export function Autocomplete<T = any>(props: IAutocompleteProps<T>) {
  ...
  const cancelFreeValueRef = React.useRef(false);
  ...
  const valueLabel = value === null || value === undefined ? undefined : String(value);
  const inputInitialValue = placeholder ?? selectedLabel ?? valueLabel;
  ...
  const onSelectFreeValue = () => {
    if (cancelFreeValueRef.current) {
      cancelFreeValueRef.current = false;
      closeDropdown();
      return;
    }
    ...
    closeDropdown();
    setActiveIndex(-1);
    onChange?.({ index: -1, name, item: null, value: newValue });
  };
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (isEscape) cancelFreeValueRef.current = true;
